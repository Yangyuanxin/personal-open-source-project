...
Item Para_Setting_Item[] =
{
    ...
    {22, 134,  "风扇", 			WHITE, BLACK, 24, 1},
    {22, 163,  "光补偿", 		WHITE, BLACK, 24, 1},
...
/*风扇状态显示*/
void fan_status_display(uint8_t status, uint16_t color)
{
...
/*光补偿状态显示*/
void light_status_display(uint8_t status, uint16_t color)
{
...
/*灵敏度显示*/
void sensivity_display(uint8_t status, float value, uint16_t color)
{
...
void para_item_display(uint8_t enable)
{
    LCD_Ascii_Show_Para ascii_colon_str[] =
    {
        ...
        {100, 134, 125, ":", BLACK, WHITE, 24},/*显示电机冒号*/
        {100, 163, 125, ":", BLACK, WHITE, 24},/*显示光补偿冒号*/
    ...
    if(enable == 1)
    {
        ...
        fan_status_display(para_setting_page_ui.fan, WHITE);
        light_status_display(para_setting_page_ui.light_compensatory, WHITE);
...
void para_setting_para_init(void)
{
    ...
    para_setting_page_ui.fan = User_Memory_Para.fan ;
    para_setting_page_ui.light_compensatory = User_Memory_Para.light_flag ;
...
void select_para_item(int item)
{
    ...
    /*风扇*/
        ...
        fan_status_display(para_setting_page_ui.fan, RED);
    /*光补偿*/
    else if(2 == item)
        light_status_display(para_setting_page_ui.light_compensatory, RED);
    /*灵敏度*/
    else if(3 == item)
        sensivity_display(para_setting_page_ui.sensivity, \
...
void setting_current_select_item(void)
{
    switch(para_setting_page_ui.select_item)
    {
        ...
        case 1:
            (para_setting_page_ui.fan < 1) ? (para_setting_page_ui.fan++) : (para_setting_page_ui.fan = 0);
            fan_status_display(para_setting_page_ui.fan, RED);
            break ;

        case 2:
            (para_setting_page_ui.light_compensatory < 1) ? (para_setting_page_ui.light_compensatory++) : (para_setting_page_ui.light_compensatory = 0);
            light_status_display(para_setting_page_ui.light_compensatory, RED);
            break ;
...
void para_save_and_return_prev_page(void)
{
    ...
    setting_fan(para_setting_page_ui.fan);
    setting_light(para_setting_page_ui.light_compensatory);
