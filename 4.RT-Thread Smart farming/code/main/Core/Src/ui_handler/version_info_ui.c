...
/*版本信息LOGO显示*/
#define VERSION_LOGO	"0:/UI/start_logo/version_logo.bmp"
...
LCD_Bmp_Show_Para version_logo_para = {0, 130, VERSION_LOGO};
LCD_Fill_Para vesion_logo_hide = {0, 240, 130, 130 + 48, BLACK};
...
LCD_Ascii_Show_Para ascii_show_para[] =
{
    ...
    {VERSION4_TEXT_START_X, VERSION4_TEXT_START_Y, 240, version_para_info.Build_Version, BLACK, RED, VERSION4_TEXT_FONT},
    /*隐藏*/
    ...
    {VERSION4_TEXT_START_X, VERSION4_TEXT_START_Y, 240, version_para_info.Build_Version, BLACK, BLACK, VERSION4_TEXT_FONT},
};
...
void display_version_info(int enable)
{
    ...
    memset(version_para_info.Build_Version, 0, 30);
    sprintf(kernel_version, "Power by RT-Thread nano");
    ...
    sprintf(version_para_info.Build_Version, "version:%s %s", __DATE__, __TIME__);
    Type_Value = MCU_TYPE_ADDR & 0x1f ;
    LCD_Ascii_Show_Para ascii_show_rtthread_para = {20, 200, 220, kernel_version, BLACK, WHITE, 16};
    LCD_Ascii_Show_Para ascii_hide_rtthread_para = {20, 200, 220, kernel_version, BLACK, BLACK, 16};
    ...
    if(enable == 1)
    {
        ...
        /*显示Power by RT-Thread nano*/
        lcd_model.lcd_driver->Lcd_show_bmp(version_logo_para);
        lcd_model.lcd_driver->lcd_show_ascii_str(ascii_show_rtthread_para);
    }
    else if(enable == 0)
    {
        for(int i = 4 ; i < 8 ; i++)
        ...
        /*隐藏Power by RT-Thread nano*/
        lcd_model.lcd_driver->lcd_fill(vesion_logo_hide);
        lcd_model.lcd_driver->lcd_show_ascii_str(ascii_hide_rtthread_para);
    }
}
