...
void ST7735_WriteData(uint8_t* buff, size_t buff_size)
{
    ...
    LCD_CS_0;
		...
		{
			*((uint8_t*)&hspi1.Instance->DR) = buff[i];
...
void LCD_SetGramScanWay(LCD_SCAN_DIR Scan_dir)
{
    ...
    if((MemoryAccessReg_Data & 0x10) != 1)
    {
        ...
        sLCD_DIS.LCD_Y_Adjust = 3;					//?
    }
    else
    {
        sLCD_DIS.LCD_X_Adjust = LCD_X;
        sLCD_DIS.LCD_Y_Adjust = LCD_Y + 6;	//?
    }
...
void LCD_SetWindows( POINT Xstart, POINT Ystart, POINT Xend, POINT Yend )
{
    ...
    LCD_WriteReg(0x2C);
		...
		LCD_WriteReg(0x2a);
    LCD_WriteData_8Bit(0x00);
    LCD_WriteData_8Bit(Xstart + 2);
    LCD_WriteData_8Bit(0x00);
    LCD_WriteData_8Bit(Xend + 2);

    LCD_WriteReg(0x2b);
    LCD_WriteData_8Bit(0x00);
    LCD_WriteData_8Bit(Ystart + 0);
    LCD_WriteData_8Bit(0x00);
    LCD_WriteData_8Bit(Yend + 5);
...
{
   // LCD_CS_0;
  //  LCD_SetWindows(x, y, x + w, y + h);
  //  ST7735_WriteData((uint8_t*)data, sizeof(uint16_t)*w * h);
  //  LCD_CS_1;
		if((w > x) && (h > y))
    {
        LCD_SetWindows(x, y, w, h);
				ST7735_WriteData((uint8_t*)data, w*h*2);
    }
}
