...
LCD_Bmp_Show_Para decode_logo_para[] =
{
    {24, 38, "0:/UI/decode/0.bmp"},//0
    {24, 38, "0:/UI/decode/1.bmp"},//1
};

/*显示二维码*/
void display_decode_status(uint8_t status)
{
    if(status > 1)
        return ;

    lcd_model.lcd_driver->Lcd_show_bmp(decode_logo_para[status]);
}

/*按键逻辑处理*/
static void Handle_Version_Info_Page_Left_Key(void)
{
		display_decode_status(0);
